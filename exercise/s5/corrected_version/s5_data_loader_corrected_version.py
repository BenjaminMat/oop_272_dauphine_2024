...
class YahooFinanceData:
    ...
    @classmethod
    def from_yahoo_data(cls, ticker: str, data_for_initialization: dict, history: pd.DataFrame):
        return cls(
            ticker=ticker,
            short_name=data_for_initialization.get('shortName', 'N/A'),
            sector=data_for_initialization.get('sector', 'N/A'),
            market_cap=data_for_initialization.get('marketCap', 0),
            currency=data_for_initialization.get('currency', 'N/A'),
            dividend_yield=data_for_initialization.get('dividendYield', 0.0),
            short_ratio=data_for_initialization.get('shortRatio', 0.0),
...
class YahooFinanceDataLoader:
    ...
    @staticmethod
    def populate_dataclass(ticker: str) -> YahooFinanceData:
        try:
            ticker_obj = YahooFinanceDataLoader.get_information_for_ticker(ticker)
            ticker_info = ticker_obj.info
            price_history = YahooFinanceDataLoader.historical_price(ticker)

            return YahooFinanceData.from_yahoo_data(ticker, ticker_info, price_history)

        except Exception:  # This is a general catch-all for exceptions not specified above
            raise MarketDataDownloadError(YahooFinanceDataLoader, 'populate_dataclass', ticker)
